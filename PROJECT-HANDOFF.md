...
JS：`var scenes={chain:'s-chain',basics:'s-basics',eda:'s-eda',wafer:'s-wafer',tr:'s-tr',fab:'s-fab',litho:'s-litho',etch:'s-etch',clean:'s-clean',adv:'s-adv',advstruct:'s-advstruct',mem:'s-mem',bepkg:'s-bepkg',pkg:'s-pkg',pwr:'s-pwr',family:'s-family'}`。默认 A 组/全链条。**已支持 hash 深链**：`principles/#eda` 切页；**2026-07-17 起支持两段式 `principles/#etch:mk-har`**（切页后再滚到小节锚点，auto+80/500ms 双次兜底）。机制加深块 subttl 已加 12 个 `mk-*` 锚点 id：mk-thermal/mk-implant/mk-rc（fab）· mk-hkmg（advstruct）· mk-scanner（litho）· mk-har/mk-ald（etch）· mk-metro（clean）· mk-nand/mk-dram（mem）· mk-advpkg（pkg）· mk-opc（wafer 掩膜写版小节）。轨2 六赛道护城河"机制证据链"共 21 条深链回轨1。**覆盖度体检**：见 `COVERAGE-AUDIT.md`（八大工艺已全部 ✅；P1 热工艺与掺杂、P2 打包待做，登记在 TRACK1-BACKFILL #7/#8）。
...
- **护城河机制证据链（2026-07-17，轨2 做深①）**：六个 `-moat` 子页在六卡阵后各加「机制 → 壁垒 · 证据链」块（CSS 类 `.mchain`：左"机制事实"⇒右"壁垒后果"两栏，「看机制 ↗」按钮深链轨1 `principles/#页:mk-锚点`，共 21 行）。内容为轨1 八个机制加深块的定性引用，快变数字一律指向既有单一事实源，未新增 FACT-REVIEW 项。顺手纠错：设备"精度即良率"卡"亚埃级"→"原子层级（ALE 逐层去除）"。
- **龙头深度卡片（2026-07-17，轨2 做深②·设备赛道试点）**：设备 e-players 子页尾 `#e-deepcards`（CSS `.dcard` 系列+中性条色 `.fill.gn`）＝国际锚对照表（ASML/AMAT/Lam × 北方华创/中微，FY2025 七列+坐标系三读数）+ 两张三段式卡（①业务拆分条形 ②护城河·机制链轨1 ③财务特征三年一期表+"报表怎么读"）。数据源 tushare+yfinance+年报交叉；市值/PE=2026-07-16 收盘快照；登记 FACT-REVIEW #22。**模板已跑通，横推其余五赛道时照此三段式**。
- **材料赛道深度卡片（2026-07-17 同日横推）**：m-players 子页尾 `#m-deepcards`＝SUMCO/Entegris 锚定表＋沪硅（周期底部重资产样本）/安集（配方成长样本）两卡——"两种报表长相"对照结构；SUMCO FY2025 亏损、折旧≈收入 28% 为硅片重资产论的行业级佐证。
- **晶圆制造深度卡片（2026-07-17 同日第三站）**：f-players 子页尾 `#f-deepcards`＝台积电/GFS 锚定表（主题"毛利率断层＝代差的财务形态"：59.9/26.1/21.6/18.7）＋中芯卡（中国区 85.8%、在建≈固定资产 68%、合资少数股东结构）＋华虹卡（特色工艺路线；少数股东损益揭示合并整体净利为负——"只看归母会系统性高估"）。
- **存储深度卡片（2026-07-17 同日第四站）**：r-players 子页尾 `#r-deepcards`＝美光锚（三年毛损→暴利的周期振幅；股价一年 +7 倍）＋"长存/长鑫未上市"结构提示＋兆易卡（利基弹性：26Q1 收入 +119%、毛利率 57.1%）＋君正卡（车规稳态：+47% 滞后拐点）——同赛道两种脾气对照。
- **封测深度卡片（2026-07-17 收官站）**：p-players 子页尾 `#p-deepcards`＝ASE/Amkor 锚（"全行业共享一个毛利率天花板"：13–18% 窄带＝国产已攻入的报表证据）＋长电卡（华润入主/2022 高点未收复/研发 ~5% 服务业属性）＋通富卡（AMD 占收入 52.3%——"研究通富先研究 AMD"；归母三年 7 倍＝稼动率杠杆；放量不提价）。**做深② 五站全部收官**。
- **EDA 深度卡片（2026-07-17 第六站，六赛道齐）**：d-players 子页尾 `#d-deepcards`＝SNPS/CDNS 锚（"同样的毛利率，差一个量级的生态"：四家毛利率同在 80–90% 带，差距在规模与研发绝对额；A 股定价尺度换轨到 PS 44×/39×）＋华大九天卡（"90% 毛利+65–84% 研发费率＝扣非持续为负"）＋概伦卡（器件建模"翻译官"+标准卡位）。**做深② 六站全部完成：6 赛道 12 家 A 股 × 14 家国际锚**。
- 校验基线（2026-07-17 六站齐）：43 sections / 9 views · div 1332/1332 平衡 · 40 表全闭合 · 无重名 id · JS 通过 · 真机渲染零 console error · 46 条 principles 深链全部有效（脚本核对）· 6 个 `#x-deepcards` 区块。atlas.html ≈287KB。
...
**A2 · "做深"阶段（2026-07-14 启动，进度见 COVERAGE-AUDIT §七）**：①ALD/高深宽比刻蚀 · ②量测检测 · ③3D NAND 专线 · ④HKMG · ⑤先进封装工艺线 · ⑥DRAM 专线 · ⑦互连 RC 与新金属 · ⑧光刻机整机——**两批八个深挖主题全部完成（07-14/15）**。轨1 机制层可深挖的主要缺口已基本收敛。

**A3 · 轨2"做深"阶段（2026-07-17 启动，进度见 COVERAGE-AUDIT §八）**：①"竞争格局机制化改写"✅（六赛道护城河机制证据链，21 条深链回轨1）；②"龙头公司基本面深度卡片"**六站全部完成（设备/材料/晶圆制造/存储/封测/EDA，均 2026-07-17，12 家 A 股 × 14 家国际锚，见 §5 与 COVERAGE-AUDIT §八）——轨2 做深②收官**。**下一步候选（按优先级）**：(a) 深度卡片 2026Q3 季报季统一刷新（FACT-REVIEW #22，与速查表 #7 联动——半年报 8 月底集中披露后做）；(b) FACT-REVIEW 2026Q4 到期批量复核（#1/2/3/5/6/9/10/12/15/19 共 10 条）；(c) 轨2 新做深方向与用户对齐后启动（§B 搁置项勿主动提议）；每赛道 2–3 家、复用三段式与取数配方（tushare fina_mainbz/income/fina_indicator/balancesheet/daily_basic + yfinance 锚），市值快照与 FACT-REVIEW #22/#7 联动刷新。其余候选：FACT-REVIEW 到期复核（下批 2026Q4）；§B 搁置项经确认后启动。
